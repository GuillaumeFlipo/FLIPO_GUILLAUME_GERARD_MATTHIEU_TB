...
class sheet:
	...
	def regressionfinale(self):
		...
		self.Y = self.newdf.Nwithdrawal_binary
		self.X = self.newdf >> select(X.lagged_NW,X.FSW1,X.FSW2,X.SAS_GPL,X.SAS_NCG,X.SAS_NBP)
		print( "taille de la dataFrame" ,self.X.tail())
		self.x_train_lrBalance, self.x_test_lrBalance,self.y_train_lrBalance,self.y_test_lrBalance=train_test_split(self.X,self.Y)
		print("Longeur de x_train_lr après tarin_test_split sur la dataframe de longueur 1628 : ")
		print( len(self.x_train_lrBalance))
		self.lrBalance = LogisticRegression()
		self.lrBalance.fit(self.x_train_lrBalance,self.y_train_lrBalance)
		self.y_pred_binary=self.lrBalance.predict(self.x_test_lrBalance)
		# print(len(self.newdf['gasDayStartedOn'].values))
		print("longueur de y_pred : ",len(self.y_pred_binary))
		self.dict_y_pred_binary={ 'y_pred_binary' : self.y_pred_binary, 'gasDayStartedOn': self.newdf['gasDayStartedOn'].values}
		self.df_y_pred_binary=pd.DataFrame(self.dict_y_pred_binary)
		print(self.df_y_pred_binary.head())
		print(self.newdf.head())
		self.dfBalance=pd.concat(self.newdf,self.df_y_pred_binary,axis=1)
		self.temp=self.dfBalance >> mask(self.y_pred_binary>0)
		self.X_reg=self.temp
		self.indexNames = self.X_reg[ self.X_reg['Nwithdrawal_binary'] == 0 ].index
		self.dflinearBalance=self.X_reg
		self.dflinearBalance.drop(self.indexNames , inplace=True)
		self.YlinearBalance = self.X_reg.NW
		self.XlinearBalance = self.X_reg >> select(X.lagged_NW,X.FSW1,X.FSW2,X.SAS_GPL,X.SAS_NCG,X.SAS_NBP)
		...
		self.lnrBalance = LinearRegression()
		self.lnrBalance.fit(self.x_train_lnrBalance,self.y_train_lnrBalance)
		self.y_pred_num=self.lnrBalanceBalance.predict(self.x_test_lnrBalance)
		self.temp=concat(temp["gasDayStartedOn"],y_pred_num, axis=1)
		self.dfBalance = pd.merge(dfBalance, temp, on = "gasDayStartedOn", how  = "outer")
		self.dfBalance = dfBalance.fillna(0)
		self.confusion_lrBalance=confusion_matrix(self.y_test_lrBalance,self.y_pred_binary)
		self.probs_lrBalance=self.lrBalance.predict_proba(self.x_test_lrBalance)[:,1]
		cm=self.confusion_lrBalance
		self.dictMetricBalance={'recall': recall_score(self.y_test_lrBalance, self.y_pred_binary), 'neg_recall': cm[1,1]/(cm[0,1] + cm[1,1]), 'confusion': cm, 'precision': precision_score(self.y_test_lrBalance, self.y_pred_binary), 'neg_precision':cm[1,1]/cm.sum(axis=1)[1], 'roc': roc_auc_score(self.y_test_lrBalance, self.probs_lrBalance)}
		print(self.dictMetricBalance)
...
#On part du principe qu'on travaille sur un seul sheet, qu'on a éventuellement récupéré à partir de la classe sheet

# def __init__(self, dataFrameName, storage_data):
# 		self.stockage = storage_data
# 		self.dfname=dataFrameName
# 		self.len=len(self.df.full)



		#On vient de récupérer l'intégralité du tableau d'un sheet stockage ainsi que les prix associés
